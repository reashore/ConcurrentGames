...
namespace SportsIq.Analytica.Ncaabb
{
    public interface IAnalyticaNcaabb
    {
    ...
    public class AnalyticaNcaabb : AnalyticaBase, IAnalyticaNcaabb
    {
        ...
        private Dictionary<int, List<Market>> RunPlayerModel(Dictionary<string, Dictionary<string, double>> modelData, Dictionary<int, List<Market>> periodMarkets, bool started, string saveFileName)
        {
            ...
            try
            {
                Dictionary<string, string[]> tableIndicesDictionary = new Dictionary<string, string[]>
                    {
                        {NcaabbModelDataKeys.InMlf, new[] {"iGP", "iBL", "iGL2"}},                                // Market Odds
                        //{NcaabbModelDataKeys.Evs, new[] {"iTVH", "iGP"}},                                       // Game State
                        //{NcaabbModelDataKeys.Egt, new[] {"iGT"}},                                               // Score
                        {NcaabbModelDataKeys.Gsc,   new[] { "iTVH", "iMTR", "iQT", "iPLY" }},                     // Game score
                        {NcaabbModelDataKeys.Ttm,   new[] { "iTVH", "iGP", "iMTR" }},                             // Team Totals by Metric 
                        {NcaabbModelDataKeys.Posc,  new[] { "iTVH", "iMTR", "iQT", "iGT", "iPLY", "iTS" }},       // Standard Deviation (player) vs Team by Metric
                        {NcaabbModelDataKeys.Potm,  new[] { "iTVH", "iMTR", "iQT", "iGT", "iPLY", "iTS" }},       // Standard Deviation (player) vs Team by Metric
                        {NcaabbModelDataKeys.Pop,   new[] { "iTVH", "iMTR", "iQT", "iGT", "iPLY", "iTS" }},       // Standard Deviation (player) vs Team by Metric
                        {NcaabbModelDataKeys.Psco,  new[] { "iTVH", "iMTR", "iQT", "iGT", "iPLY", "iTS" }},       // Standard Deviation (player) vs Team by Metric
                        {NcaabbModelDataKeys.Sdvtm, new[] { "iTVH", "iMTR", "iPLY" }},                            // Standard Deviation (player) vs Team by Metric
                        {NcaabbModelDataKeys.Sdom,  new[] { "iTVH", "iMTR", "iPLY" }}                             // Standard Deviation (player) vs Team by Metric
                    };
        ...
        private Dictionary<int, List<Market>> RunTeamModel(Dictionary<string, Dictionary<string, double>> modelData, Dictionary<int, List<Market>> periodMarkets, bool started, string saveFileName)
        {
             ...
             */
            if (!modelData[NcaabbModelDataKeys.Settings].ContainsKey("MSR"))
            {
                modelData[NcaabbModelDataKeys.Settings]["MSR"] = 0.0;
            }

            if (!modelData[NcaabbModelDataKeys.Settings].ContainsKey("SCW"))
            {
                modelData[NcaabbModelDataKeys.Settings]["SCW"] = 0.0;
            }

            if (!modelData[NcaabbModelDataKeys.Settings].ContainsKey("FOL"))
            {
                modelData[NcaabbModelDataKeys.Settings]["FOL"] = 1;
            }

            Dictionary<string, double> tempSettings = new Dictionary<string, double>
            {
                {"MSR", modelData[NcaabbModelDataKeys.Settings]["MSR"]},
                {"SCW", modelData[NcaabbModelDataKeys.Settings]["SCW"]}
            };
            UpdateSettings(tempSettings);

            _followLive = modelData[NcaabbModelDataKeys.Settings]["FOL"].IsEqualTo(1);

            try
            {
                Dictionary<string, string[]> tableIndicesDictionary = new Dictionary<string, string[]>
                    {
                        {NcaabbModelDataKeys.InMlf, new[] {"iGP", "iBL", "iGL2"}},            //good
                        {NcaabbModelDataKeys.InLMlf, new[] {"iGP", "iBL", "iGL2"}},           //good
                        {NcaabbModelDataKeys.InSc,  new[] {"iTVH", "iQT", "iGT"}},            //
                        {NcaabbModelDataKeys.InLsF,  new[] {"iQT", "iGT"}},
                        {NcaabbModelDataKeys.InTss, new[] {"iTVH", "iAR", "iSHS" }},
                        {NcaabbModelDataKeys.InTsf, new[] {"iTVH", "iQT", "iSAS",  "iGT"}},
                        {NcaabbModelDataKeys.Evs, new[] {"iTVH", "iGP"}},                     // Game State
                        {NcaabbModelDataKeys.Egt, new[] {"iGS"}}
                    };

                Dictionary<string, double> tempDictionary = Utils.CopyDictionary(modelData[NcaabbModelDataKeys.InMlf]);
                modelData[NcaabbModelDataKeys.InMlf] = ValidateMarketOdds(modelData[NcaabbModelDataKeys.InMlf]);
                LoadTables(tableIndicesDictionary, modelData);
                modelData[NcaabbModelDataKeys.InMlf] = tempDictionary;

                try
                {

                    if (modelData.ContainsKey(NcaabbModelDataKeys.InLMlf))
                    {
                        bool arePeriodMarketsValid2 = modelData[NcaabbModelDataKeys.InLMlf]["CG,SP,S1"].IsNotEqualToZero();
                        bool arePeriodMarketsValid3 = modelData[NcaabbModelDataKeys.InLMlf]["CG,ML,S1"].IsNotEqualToZero();
                        bool arePeriodMarketsValid1 = modelData[NcaabbModelDataKeys.InLMlf]["CG,TO,T"].IsNotEqualToZero();
        ...
        {
            if (!modelData.ContainsKey(NcaabbModelDataKeys.InMlf))
            {
                return false;
            }

            Dictionary<string, double> inMlf = modelData[NcaabbModelDataKeys.InMlf];
        ...
        public Dictionary<string, List<string>> GetAllTableIndices()
        {
            ...
            {
                NcaabbModelDataKeys.InMlf,
                NcaabbModelDataKeys.Gsc,
                NcaabbModelDataKeys.Ttm,
                NcaabbModelDataKeys.Posc,
                NcaabbModelDataKeys.Potm,
                NcaabbModelDataKeys.Pop,
                NcaabbModelDataKeys.Psco,
                NcaabbModelDataKeys.Sdvtm,
                NcaabbModelDataKeys.Sdom
            };
